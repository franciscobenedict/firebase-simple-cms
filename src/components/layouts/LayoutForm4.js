...
const LayoutForm4 = (props) => {
  ...
  useEffect(() => {
    if (props.currentId === ''){
      ...
      Object.keys(props.contentObjects).map((key) => (
        key === props.currentId ? setUrl(props.contentObjects[key].imageLeft) : ''
      ))
    }
      ...
      uploadTask.on(
        ...
        () => {
          storage.ref("images/content/layoutImages").child(image.name).getDownloadURL().then(url => {
            setUrl(url);
            setProgress(0);
            values.imageLeft = url;
            console.log('values.imageLeft', values.imageLeft);
            console.log('url', url);
  ...
  return (
    <form autoComplete="off" onSubmit={handleFormSubmit} className="layout_form layout">
      <div className="form_subheading">Layout 4</div>
